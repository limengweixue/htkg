...
class GetAllJob(Resource):
    @staticmethod
    def get():
        """
        get all job info
        """
        dirlist = os.listdir(running_path)
        status_list = []
        for dir in dirlist:
            if re.match(r".+?_status", dir):
                sid = dir.split("_")[0]
                job = Running(sid)
                status = job.status()
                data = job.list()
                if status['status'] == 'running':
                    ret = True
                    status['still_running'] = dict()
                    for s_sid, git in data['sids'].items():
                        if Running(s_sid).is_file(True) is False:
                            status['still_running'].update({s_sid: git})
                            ret = False
                    if ret:
                        status['status'] = 'done'
                        job.status(status)
                status_list.append({
                    "taskId": sid,
                    "taskDescription": status.get("desc"),
                    "executingState": "已经完成" if status.get("status") == "done" else "未完成",
                    "operate": "",
                    "high":"0"})
        return {"code": 1001, "result": status_list}
...
class AddJob(Resource):
    ...
    def post():
        ...
        if not target or target == "":
            return {"code": 1002, "msg": " python  flask  nginx logURL cannot be empty."}
...
class Summary(Resource):
    ...
    def post():
        ...
        if running.is_file() is not True:
            ...
            scan_status_file = os.path.join(running_path, '{sid}_status'.format(sid=sid))
            ...
            for s_sid, target_str in r_data.get('sids').items():
                    target_info = dict()
            ...
            data = {
                ...
                "totalpreview": {
                    "sumarry": {
                        ...
                        "totalNumber": total_targets_number,
                        "target": len(targets)
                    },
                    "vulaudit": {
                        "Critical": critical_vul_number,
...
class Query(Resource):
    ...
    def post():
        ...
        ret=[]
        search = {}
        for id, vul in rules.items():
            typeid = int(str(id)[:3])
            if not search.get(classes[typeid]):
                search[classes[typeid]] = []
            if vul not in search[classes[typeid]]:
                search[classes[typeid]].append([id,vul])
        ...
        # return {"code":1001,"result":{"search":search}}

        return {"code": 1001, "result": {"search": {"option3": ret}}}


class ScanObj(Resource):
    @staticmethod
    def post():
        data = request.json
        if not data or data == "":
        ...
        for s_sid, target_str in r_data.get('sids').items():
            if s_sid not in still_running:
                ...
                data = Running(s_sid).data()["result"]
                target_info.update({
                        ...
                        "PushedRules": data.get("push_rules"),
...
def start(host, port, debug):
    ...
    resource.add_resource(Report, '/api/report')
    resource.add_resource(GetAllJob, '/api/jobs')
    resource.add_resource(Summary, '/api/summary')
    resource.add_resource(ScanObj, '/api/scanobj')
    resource.add_resource(Query, '/api/query')
